...
export default function AdminCategoriesPage() {
  ...
  const updateImage = async (id: string, imageUrl: string) => {
    await fetch("/api/admin/categories", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, imageUrl }) });
    load();
  };
          ...
          <li key={c.id} className="flex items-center justify-between gap-3 p-3 text-sm">
            <div className="flex items-center gap-3">
              <MediaUploader
                ...
                onChange={(media) => updateImage(c.id, media.url)}
              />
